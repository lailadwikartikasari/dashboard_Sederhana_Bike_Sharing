...
def main():
    ...
    min_date = merged_data_df['dteday'].min().date()  # .date() untuk mendapatkan tipe data date
    max_date = merged_data_df['dteday'].max().date()  # .date() untuk mendapatkan tipe data date
    start_date, end_date = st.sidebar.date_input("Pilih Rentang Tanggal", [min_date, max_date], min_value=min_date, max_value=max_date) # Menambahkan min_value & max_value
    ...
    merged_data_df = merged_data_df[(merged_data_df['dteday'] >= pd.to_datetime(start_date)) & 
    ...
    if 'season' in merged_data_df.columns:
        ...
        # Mengubah default value pada multiselect menjadi [] agar tidak langsung menfilter semua data
        selected_season = st.sidebar.multiselect("Filter Musim", merged_data_df['season_cat'].unique(), default=[])
    ...
    if 'dteday' in merged_data_df.columns and 'cnt' in merged_data_df.columns:
        ...
        # Filter daily_df berdasarkan rentang tanggal
        daily_df = daily_df[(daily_df['dteday'] >= pd.to_datetime(start_date)) & 
                                (daily_df['dteday'] <= pd.to_datetime(end_date))]
